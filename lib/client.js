window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var _dsh_css_waterball_module_css_default = {
			...
			"float": "m0Tjba_float",
			"halo": "m0Tjba_halo",
			"halo-authorizing": "m0Tjba_halo-authorizing",
		...
		*/
		...
		function WaterballPet({ persistPosition }) {
			...
			const mood = wave ? "waving" : status?.mood === "failed" ? "failed" : status?.mood === "stopped" ? "stopped" : status?.mood === "done" ? "done" : status?.mood === "jumping" ? "jumping" : status?.mood === "waiting" ? "waiting" : status?.mood === "authorizing" ? "authorizing" : "idle";
